...
function getNewLatestPosts() {
    ...
    if(latestPosts[keyword].length > calc) {
        $latestPosts.innerHTML = createTemplatesForLatestPosts(latestPosts[keyword].slice(calc, calc + 5))
    }
    ...
    fetch(`https://pixabay.com/api/?key=${key}&q=${keyword}&image_type=photo&order=latest&page=${countPage}&per_page=5`)
        ...
        .then( data => {
            ...
            $latestPosts.innerHTML = createTemplatesForLatestPosts(data.hits, false)
        })
}